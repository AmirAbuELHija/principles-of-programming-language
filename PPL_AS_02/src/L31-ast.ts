...
import { Result, makeOk, makeFailure, bind, mapResult, mapv, safe2, isOk } from "../shared/result";
...
;;         |  ( cond <CondClause>+ <ElseClause> )  / CondExp(CondClause:List(CondClause))
;;         |  ( quote <sexp> )              / LitExp(val:SExp)
;;         |  ( <cexp> <cexp>* )            / AppExp(operator:CExp, operands:CExp[]))
;; <binding>  ::= ( <var> <cexp> )           / Binding(var:VarDecl, val:Cexp)
;; <CondClause> ::= (<cexp> <cexp>+)        /  CondClause(test: CExp, then: CExp[])
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | CondExp | CondClause | ElseClause;
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isCondExp(x) || isElseClause(x) || isCondClause(x);
...
const isSpecialForm = (x: string): boolean =>
    ["if", "cond", "lambda", "let", "quote"].includes(x);
...
const parseCondExp = (params: Sexp[]): Result<CondExp> => {
    if(params.length < 2){
        return makeFailure("missing case");
    }
    if (!allT(isCompoundSexp, params)) {
        return makeFailure("Invalid cond clauses");
    } else {
        const elseClause = params[params.length - 1][0];
        if(elseClause != "else"){
            return makeFailure("cond doesnt contain else")
        }
        const clausesResult = mapResult((clause: Sexp[]) =>
            safe2((test: CExp, then: CExp[]) => makeOk(makeCondClause(test, then)))
                 (parseL31CExp(first(clause)), mapResult(parseL31CExp, rest(clause))), params.slice(0,-1));

        const clauseWithoutElse = params[params.length - 1].slice(1);
        const parsedClauseWithoutElse = parseL31CExp(clauseWithoutElse);
        if(isOk(parsedClauseWithoutElse)){
            const ElseValue = parsedClauseWithoutElse.value;
            return bind(clausesResult, (clauses: CondClause[]) => makeOk(makeCondExp(clauses , makeElseClause(ElseValue))));
        }else{
            return makeFailure("is not ok after parsing the else clause");
        }
...
const unparseCondExp = (condclauses: CondClause[]): string => 
        map((condclause: CondClause) => unparseL31(condclause), condclauses).join(" ");
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isCondExp(exp) ? `(cond ${unparseCondExp(exp.condclauses)} ${unparseL31(exp.elseClause)})` :
    isCondClause(exp) ? `(${unparseL31(exp.test)} ${map((cexp: CExp) => unparseL31(cexp), exp.then)})` :
    isElseClause(exp) ? `(else ${unparseL31(exp.then).slice(1,-2)})` :
