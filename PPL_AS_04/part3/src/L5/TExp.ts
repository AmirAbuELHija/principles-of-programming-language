...
*/
import { chain, concat, equals, map, union, uniq } from "ramda";
...
export const makeUnionTExp = (components:TExp[]): UnionTExp =>
 ({tag:"UnionTExp" ,components:components });
...
*/
export const parseTExp1 = (texp: Sexp): Result<TExp> =>
    (texp === "number") ? makeOk(makeNumTExp()) :
    (texp === "boolean") ? makeOk(makeBoolTExp()) :
    (texp === "void") ? makeOk(makeVoidTExp()) :
    (texp === "string") ? makeOk(makeStrTExp()) :
...
const parseUnion = (te: Sexp[]): Result<TExp> => {
  const components: TExp[] = [];
  if(te.length >= 1){
    for (let i = 0; i < te.length; i++) {
      const component_Result = parseTExp1(te[i]);
      if (isFailure(component_Result)) {
        return component_Result;
    }
    const component = component_Result.value;
    if (component.tag === "UnionTExp") {for (const subComponent of component.components) { add_Unique_Component_if_not_exist(components, subComponent); }} else { add_Unique_Component_if_not_exist(components, component);}
  }
  return makeOk(makeUnionTExp(components.sort(compareTExp)));
}
else{ return makeFailure("missing arguments");  }
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> => {
  ...
  return pos === -1 ?
  texps[0] === "union" ? parseUnion(texps.slice(1)) :
  makeFailure(`Error in format ${format(texps)}`) :
...
const add_Unique_Component_if_not_exist = (components: TExp[], component: TExp) => {
    if (!components.some((comp) => equals(comp, component))) {
...
const parseTupleTExp = (texps: Sexp[]): Result<TExp[]> => {
    ...
    // [x1 * x2 * ... * xn] => [x1,...,xn]
    if (texps[0] === 'union') {
        return bind(mapResult(parseTExp, texps.slice(1)), (components: TExp[]) =>
          makeOk([makeUnionTExp(components)])
        );
    }
...
// export const unparseUnionTExp(x: UnionTExp): Result<string | string[]> =>
const unparseUnionTExp = (te: UnionTExp): Result<string> => {
    const unparse_the_Component = (comp: TExp): Result<string> =>
      isUnionTExp(comp) ? unparseUnionTExp(comp) : unparseTExp(comp);
    
    const componentsLength = te.components.length;
    if (componentsLength === 2 || componentsLength === 1 ) {
      return bind(mapResult(unparse_the_Component, te.components), (unparsedComponents: string[]) => {
        const joinedComponents = unparsedComponents.join(" ");
        return makeOk(`(union ${joinedComponents})`);
      });
    } else if (componentsLength > 2) {
      return bind(mapResult(unparse_the_Component, te.components), (unparsedComponents: string[]) => {
        const last_Component = unparsedComponents.pop();
        const before_Last=unparsedComponents.pop();
        const joined_Components = unparsedComponents.join(" ");
        const nested_Union = `(union ${before_Last} ${last_Component})`;
        return makeOk(`(union ${joined_Components} ${nested_Union})`);
      });
    } else {
      return makeFailure("UnionTExp  Error");
    }
  };
...
export const unparseTExp = (te: TExp): Result<string> => {
  ...
  const up = (x?: TExp): Result<string | string[]> =>
    ...
    isNonEmptyTupleTExp(x) ? unparseTuple(x.TEs) :
     x === undefined ? makeFailure("Undefined TVar"): x;
