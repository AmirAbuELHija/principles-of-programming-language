...
import { endsWith, map, repeat, zipWith } from "ramda";
...
const evalBound = (exp: BoundExp, env: Env): Result<boolean> => {
    const varValueResult = applicativeEval(exp.var, env);
    if (isFailure(varValueResult)) {
      return makeOk(false);
    }
    const varValue = varValueResult.value;
    return makeOk(varValue !== undefined);
};
...
const evalTime = (exp: TimeExp, env: Env): Result<CompoundSExp> => {
    ...
    const result = bind(applicativeEval(exp.exp, env), (value: Value) => {
        const time2 = Date.now(); 
        const timeDiff = time2 - time1; 
        const timePair: CompoundSExp = makeCompoundSExp(value, timeDiff);
        return makeOk(timePair);
    });
    return result;
  };
