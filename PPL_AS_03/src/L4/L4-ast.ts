...
;; | ( bound? <var> ) / BoundExp(var: varRef) #### L4
;; | ( time <var> ) / TimeExp(var: varRef) #### L4
...
export const makeBoundExp = (v: VarRef): BoundExp =>
 ({tag: "BoundExp", var : v});
export const makeTimeExp = (v: CExp): TimeExp =>
 ({tag: "TimeExp" , exp : v});
...
// HW3: Complete these functions.
const parseBoundExp = (params: Sexp[]): Result<BoundExp> =>
 isNonEmptyList<Sexp>(params) ?
 (params.length > 1) ? makeFailure(`bound! has too many arguments: format(params)`) :
 parseGoodBoundExp(first(params)):
 makeFailure("Bound! missing arguments");

const parseGoodBoundExp = (variable: Sexp): Result<BoundExp> =>
 !isIdentifier(variable) ? makeFailure(`arg of Bound! must be an identifier: ${format(variable)}`) :
 makeOk(makeBoundExp(makeVarRef(variable)))

const parseTimeExp = (params: Sexp[]): Result<TimeExp> => 
isNonEmptyList<Sexp>(params) ?
 (params.length != 1) ? makeFailure(`bound! has too many or too low arguments: format(params)`) :
 parseGoodTimeExp(first(params)):
makeFailure("time missing arguments");

const parseGoodTimeExp = (variable: Sexp): Result<TimeExp> =>{
 const amer = parseL4CExp(variable)
 if(isOk(amer) ){
 
 return makeOk(makeTimeExp(amer.value))
 }
 return makeFailure("a")
}
...
const unparseBoundExp = (se: BoundExp): string =>
 `(bound? ${unparse(se.var)})`
const unparseTimeExp = (se: TimeExp): string =>
 `(time ${unparse(se.exp)})`
...
export const unparse = (exp: Parsed): string =>
 ...
 isBoundExp(exp) ? unparseBoundExp(exp) :
 isTimeExp(exp) ? unparseTimeExp(exp) :
 exp;